  ...
  if (searchInput) {
    searchInput.addEventListener('input', () => {
      if (currentTab === 'all') {
        ...
        renderDesigns(allDesigns.filter(d => d.status === 'draft' && (d.name || '').toLowerCase().includes(searchInput.value.trim().toLowerCase())));
      } else if (currentTab === 'history') {
        const sorted = [...allDesigns].sort((a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt));
        renderDesigns(sorted.slice(0, 5).filter(d => (d.name || '').toLowerCase().includes(searchInput.value.trim().toLowerCase())));
      } else if (currentTab === 'status') {
        // Re-render status tab with search filter
        const filterBar = document.getElementById('statusFilterBar');
        const activeBtn = filterBar ? filterBar.querySelector('.status-filter-btn.active') : null;
        const status = activeBtn ? activeBtn.getAttribute('data-status') : 'approved';
        renderDesigns(allDesigns.filter(d => d.status === status && (d.name || '').toLowerCase().includes(searchInput.value.trim().toLowerCase())));
      }
    });
    // Optional: handle Enter key to prevent form submit
