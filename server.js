...
app.post('/api/create-payos-order', async (req, res) => {
  ...
  try {
    ...
    // Generate a proper order code for PayOS (must be a number < 9007199254740991)
    if (!orderCode || typeof orderCode === 'string') {
      // Create a smaller, unique order code
      const timestamp = Date.now() % 1000000000; // Use last 9 digits of timestamp
      const random = Math.floor(Math.random() * 10000); // 4 digit random number
      orderCode = timestamp * 10000 + random; // Combine to create unique number
    }
    
    // Ensure orderCode is a number and within PayOS limits
    if (typeof orderCode === 'string') {
      orderCode = parseInt(orderCode.replace(/\D/g, ''), 10);
    }
    
    // Validate orderCode is within PayOS limits
    if (orderCode > 9007199254740991 || orderCode <= 0) {
      // Generate a new valid order code
    ...
    const checksumKey = process.env.PAYOS_CHECKSUM_KEY || 'f78c58627c4a3a7345f69488e43010f3ec6207344b5734fc9c082a2872694952';
    ...
    const response = await axios.post('https://api-merchant.payos.vn/v2/payment-requests', {
      ...
      headers: {
        'x-client-id': process.env.PAYOS_CLIENT_ID || '12fdcdb7-433f-4230-aa68-505ce0b8dbae',
        'x-api-key': process.env.PAYOS_API_KEY || '10b4a65b-d594-4781-9bb6-c70c41c84016',
...
app.post('/api/custom-design-order', async (req, res) => {
  try {
    ...
    // Generate unique order code for PayOS (must be a number < 9007199254740991)
    const timestamp = Date.now() % 1000000000; // Use last 9 digits of timestamp
    const random = Math.floor(Math.random() * 10000); // 4 digit random number
    const orderCode = timestamp * 10000 + random; // Combine to create unique number
...
// Public API: Get user (designer or customer) info by username
app.get('/api/users/:username', async (req, res) => {
  try {
    const username = req.params.username;
    // Try to find in Designer first
    let user = await Designer.findOne({ username }).select('-password -verificationCode -resetCode');
    let role = 'designer';
    if (!user) {
      user = await Customer.findOne({ username }).select('-password -verificationCode -resetCode');
      role = 'customer';
    }
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
...
// API: Get all reviews (admin dashboard)
